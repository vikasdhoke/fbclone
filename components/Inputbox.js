            ...
            .then((doc) => {
                if (imageToPost) {
                    ...
                    uploadTask.on('state_change',   
                        ...
                        () => {storage.ref(`post`).child(doc.id).getDownloadURL().then(url => { db.collection('posts').doc(doc.id).set({
                                        postImage: url
                                    }, { merge: true })
                                })
                            })  
        ...
        <div className=' bg-white p-2 rounded-xl shadow-md text-gray-500 font-medium mt-6' >
            <div className='flex space-x-4 p-4 items-center'>
                ...
                {imageToPost && (
                    <div onClick={removeImage} className='flex flex-col filter hover:brightness-110 transition duration-150 transform hover:scale-105 cursor-pointer' >
                        <img className='h-10 object-contain layout-fill' src={imageToPost} alt='' />
            ...
            <div className='flex justify-evenly p-3 border-1'>
                <div className='inputIcon'>
                    ...
                    <p className='text-xs sm:text-5m
                    xl:text-base'>Live Vedeo</p>
                </div>
